...
namespace TiltBrush
{
    public class BrushIconAssigner : Editor
    {
        ...
        private const int kTargetIconSize = 128;

        [MenuItem("Open Brush/Screenshots/Assign Brush Icons")]
        static void AssignBrushIcons()
        {
            List<string> pngPaths = FindIconFiles();
            if (pngPaths.Count == 0)
            {
                Debug.LogError("[BrushIconAssigner] No PNG files found in " + kIconFolder);
                return;
            }
            ...
            foreach (string pngPath in pngPaths)
            {
                ...
                {
                    Debug.LogError("[BrushIconAssigner] SKIP no matching BrushDescriptor for " +
                        durableName + " (" + pngPath + ")");
                    skippedCount++;
                    continue;
                }
                ...
                if (icon == null)
                {
                    Debug.LogError("[BrushIconAssigner] SKIP could not load texture at " + pngPath);
                    skippedCount++;
                    continue;
                }
            ...
            Debug.Log("[BrushIconAssigner] Assigned " + assignedCount + ", skipped " + skippedCount);
        }
        ...
        private static List<string> FindIconFiles()
        {
            ...
            {
                Debug.LogError("[BrushIconAssigner] Folder not found: " + kIconFolder);
                return result;
            }
        ...
        private static void ConfigureImportSettings(List<string> pngPaths)
        {
            foreach (string pngPath in pngPaths)
            {
                ...
                if (importer == null)
                {
                    Debug.LogError("[BrushIconAssigner] SKIP no TextureImporter for " + pngPath);
                    continue;
                }

                importer.isReadable = true;
                importer.maxTextureSize = kTargetIconSize;
...
namespace TiltBrush
{
    public class BrushIconAssigner : Editor
    {
        private const string kIconFolder = "Assets/Resources/Brushes/BrushIcons/VideoBlack";
        private const string kFilePrefix = "brush-";
        private const string kFileExtension = ".png";
        private const int kTargetIconSize = 128;
        ...
        private static Dictionary<string, BrushDescriptor> BuildDescriptorLookup()
        {
            ...
            foreach (string guid in guids)
            {
                ...
                if (result.ContainsKey(descriptor.DurableName))
                {
                    Debug.LogError("[BrushIconAssigner] Duplicate DurableName " +
                        descriptor.DurableName + " at " + path);
                    continue;
                }
                result[descriptor.DurableName] = descriptor;
