...
#pragma warning restore 414
        ...
        static bool IsExcludedBrushIconPath(string assetPath)
        {
        ...
        static IEnumerable<Texture2D> IterTextures(TiltBrushManifest[] manifests)
        {
            foreach (var manifest in manifests)
            {
                foreach (var brush in manifest.Brushes.Concat(manifest.CompatibilityBrushes))
                {
                    ...
                    if (IsExcludedBrushIconPath(AssetDatabase.GetAssetPath(tex)))
                    {
                        continue;
                    }
                    yield return tex;
        ...
        void Pack()
        {
            try
            {
                ...
                foreach (var group in IterAssets<Texture2D>(folders: m_paths)
                    .Where(t => !IsExcludedBrushIconPath(AssetDatabase.GetAssetPath(t)))
