...
namespace TiltBrush
{
    ...
    public class CanvasScript : MonoBehaviour
    {
        ...
        private int m_UnsanctionedXformLogCount;
        ...
        void Update()
        {
            ...
            {
                m_UnsanctionedXformLogCount += 1;
                if (m_UnsanctionedXformLogCount <= 3 ||
                    (m_UnsanctionedXformLogCount % 50) == 0)
                {
                    Debug.LogError(
                        "[CanvasScript.Update] CANVAS_XFORM: unsanctioned name=" +
                        gameObject.name +
                        " count=" + m_UnsanctionedXformLogCount);
                }
                transform.hasChanged = false;
            }
#endif
