...
namespace TiltBrush
{
    ...
    public class SketchMemoryScript : MonoBehaviour
    {
        ...
        public float RepaintProgress
        {
            get
            {
                return m_RepaintCoroutine == null ? 1f : m_RepaintProgress;
            }
        }
        ...
        public TrTransform InitialSketchTransform
        {
            get
            {
                return m_xfSketchInitial_RS;
            }
            set
            {
                m_xfSketchInitial_RS = value;
            }
        }
        ...
        public bool MemoryWarningAccepted
        {
            get
            {
                return m_MemoryWarningAccepted;
            }
            set
            {
        ...
        public float MemoryExceededRatio
        {
            get
            {
                return (float)m_LastCheckedVertCount / (float)m_MemoryWarningVertCount;
            }
        }
        ...
        public bool IsMemoryDirty()
        {
            if (m_OperationStack.Count != m_LastOperationStackCount)
            {
                ...
                bool dirty = newCommands.Any(e => e.NeedsSave);
                if (dirty)
                {
                    string names = "";
                    foreach (BaseCommand cmd in newCommands)
                    {
                        if (cmd == null || !cmd.NeedsSave)
                        {
                            continue;
                        }
                        if (names.Length > 0)
                        ...
                        names += cmd.GetType().Name;
                    }
                    Debug.LogError(
                        "[SketchMemoryScript.IsMemoryDirty] DIRTY: true stack=" +
                        m_OperationStack.Count +
                        " lastSaveCount=" + m_LastOperationStackCount +
                        " needsSave=" + names);
                }
                return dirty;
            }
        ...
        public bool CanUndo()
        {
            return m_OperationStack.Count > 0;
        }
        public bool CanRedo()
        {
        ...
        public void PerformAndRecordCommand(BaseCommand command, bool discardIfNotMerged = false, bool invoke = true)
        {
            ...
            Debug.LogError(
                "[SketchMemoryScript.PerformAndRecordCommand] CMD: push " +
                command.GetType().Name +
                " stack=" + m_OperationStack.Count +
                " lastSaveCount=" + m_LastOperationStackCount +
                " needsSave=" + command.NeedsSave);
            OperationStackChanged?.Invoke();
        ...
        public void RecordCommand(BaseCommand command)
        {
            ...
            m_OperationStack.Push(command);
            Debug.LogError(
                "[SketchMemoryScript.RecordCommand] CMD: push " +
                command.GetType().Name +
                " stack=" + m_OperationStack.Count +
                " lastSaveCount=" + m_LastOperationStackCount);
            OperationStackChanged?.Invoke();
