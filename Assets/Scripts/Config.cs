...
namespace TiltBrush
{
    ...
    public class Config : MonoBehaviour
    {
        ...
        // Updated for batch processing.  Evaluate if the change impacts single pass functionality (removed batch command blocker)
        void ParseArgs(string[] args)
        {
            ...
            if (isInBatchMode)
            {
                // Allow offline (--renderCameraPath) under batchmode.
                // Still block pure ODS. The two-pass offline path already
                // forces monoscopic and disables XR itself.
                if (m_SdkMode == SdkMode.Ods)
                {
                    throw new ApplicationException("ODS rendering not supported in batch mode.");
                }
                m_SdkMode = SdkMode.Monoscopic;
            }
        ...
        /*
        // ORIGINAL VERSION WITH THE BATCH MODE PROCESSING HARD BLOCK
        void ParseArgs(string[] args)
        {
            List<string> files = new List<string>();
